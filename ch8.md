...
`forEach(..)` 是遍历辅助函数, 但是它被设计为带有副作用的函数来处理每次遍历; 你会许已经猜测到了它为什么不是我们正在讨论的函数式编程列表操作！
...
有些人提倡在迭代的时候采用 `map(..)` 替代 `forEach(..)`，它本质上不会去触碰接受到的值，但却有副作用的产生：
...
如果你在一锅水里面做意大利面条，然后将这锅面条倒入滤网（过滤）中，你是过滤了意大利面条，还是过滤掉了水？ 如果你将咖啡渣放入过滤取中，然后泡一杯咖啡，你是将咖啡过滤到了杯子里，还是说将咖啡渣过滤掉？
...
在第四章中，我们讨论了 `compose(..)` 实用函数，和展示了用 `reduce(..)` 来实现的例子：
...
对于函数式编程者来说，普遍感到失望的原因是 Javascript 采用统一的策略处理实用函数，但其中的一些被采用独立函数提供。想想在前面的章节中的介绍的大量的函数式实用程序，以及另一些实用函数是数组的原型方法，就像在这章中看到的那些。
...
这就是为何函数式编程类库通常定义 `filter(..)`, `map(..)` 和 `reduce(..)` 交替采用最后一个形參接收数组，而不是第一个。它们通常自动地柯理化实用函数：
...
首先，我们可以注意到声明和运行前的一系列if语句确保了由 `getCurrentSession()`, `getSessionId(..)`, `lookupUser(..)`, `getUserId(..)`, `lookupOrders(..)` 和 `processOrders(..)` 这六个函数组合调用时的有效。理想地，我们期望摆脱这些变量定义和命令式的条件。
...
接下来，我们观察到 `getSessionId(..)` 和 `getUserId(..)` 可以看成映射对应的 `"sessId"` 和 `"uId"` 值：
