...
本文之前已经简要的提及了一些实用函数：`map(..)`, `filter(..)`, 和 `reduce(..)`，现在深入了解一下它们。在Javascript中，这些实用函数通常被用于Array（即“list”）的原型上。因此可以很自然的将这些实用函数和数组或列表操作联系起来。

曾经我们谈及这些数组方法，我们希望概念性地清楚这些操作的用途。在这章中，弄明白为何有这些列表操作和这些操作如何工作同等重要。请跟上节奏。

在本章内外，有大量这些列表操作的常见例子，它们描述一些细小的任务处理一组值（如将数组中的每一个数组加倍）。这样通俗易懂。

但是不要停留在这些简单示例的表面，而错过了更深层次的点。通过对一系列任务建模来理解一些非常重要的函数式编程在列表操作中的价值。——些看起来不像列表的语句——作为列表操作，而不是单独执行。

这不仅仅是编写许多简练代码的技巧。 我们所要做的是，从命令式转变为声明式风格，使代码模式更容易辨认，从而可读性更好。

但这里有一些**更需要掌握的**东西。在命令式代码中，一组计算的中间结果都是通过赋值来存储。代码中依赖的命令模式越多，代码中存在的错误就越难被验证。比如，在逻辑上，值的意外改变，或隐藏的潜在原因／影响。

通过且／或链接组合列表操作，中间结果被隐式地跟踪，并在很大程度上避免了这些风险。

**注意** 相比前面几章，为了代码片段更加简练，我们将采用ES6的箭头函数。尽管第二章中对于箭头函数的建议依旧普遍适用于编码中。

## 非函数式列表处理

作为本章讨论的快速预览，我想调用一些操作，这些操作看上去可以将Javascript数组和函数式列表操作有关联，但它们没有。我们不会在这里讨论这些，因为它们与一般的函数式编程最佳实践不一致：

* `forEach(..)`
* `some(..)`
* `every(..)`

`forEach(..)` 是遍历辅助函数, 但是它被设计为采用每个函数调用来处理，这样会有副作用; 你会许已经猜测到了它为什么不是我们正在讨论的函数式编程列表操作！
...
映射是将一个值转换为另一个值。例如，如果你将`2`乘以`3`，你将得到转换的结果`6`。需要重点注意的是，我们并不是在讨论映射转换是暗示就地转换或重新赋值，而是将一个值从一个地方映射到另一个新的地方。
...
但是，再一些其他情况中，你只希望传递列表项到`mapperFn(..)`。因为额外的参数可能会改变它的行为。在第三章的“共同目的（All for one）”中，我们介绍了unary(..)，它限制函数仅仅接受一个参数，不论多少个参数被传入。

回顾第三章中的关于限制`parseInt(..)`函数，仅仅采用单个参数，它也可以安全的用在`mapperFn(..)`函数中:
...
Javascript 提供了内建的数组操作方法`map(..)`，这个方法使得列表中的链式操作更为便利。

**注意** Javascript数组中的原型中定义的操作(`map(..)`, `filter(..)`, 和 `reduce(..)`)的最后一个可选参数可以被用于绑定“this”到当前函数。我们在第二章中曾经讨论过“什么是this”，以及在函数式编程的最佳实践中应该避免使用`this`。基于这个原因，在这章中的示例中，我们不采用`this`绑定功能。
...
我们注意到关于`map(..)`的一些有趣的事情：我们通常假定列表是从左往右执行的，但`map(..)`没有这个概念，它不确实不需要这个次序。每一个转换应该独立于其他的转换。

映射普遍适用于并行处理的场景中，尤其在处理大列表时可以提升性能。但是在Javascript中，我们并没有看到这样的场景。因为这里不需要你传入诸如`mapperFn(..)`这样的纯函数，即便你**应当这样做**。 如果传入了非纯函数，JS在不同的顺序中执行不同的方法，这将很快产生大问题。

尽管从理论上讲，单个映射操作是独立的，JS认为它们不是。这是令人讨厌的。
...
我们建议，数组以及在数组上应用的数组操作都是迫切的同步的，然而，这些相同的操作也可以应用在一直接受新值的“延迟列表”（即流）上。 我们将在第十章中深入讨论它。

### 映射 vs Eaching

有些人提倡在迭代的时候采用`map(..)`替代`forEach(..)`，它本质上将接受到的值完全不接触的予以通过，但随后会产生一些副作用：
...
..
```
这种技术似乎非常有用的原因是`map(..)`返回数组，这样你可以在它之后继续链式执行更多的操作。而`forEach(..)`返回的的值是`undefined`。然而，为认为你应当避免采用这种方式使用`map(..)`，因为这明显的以非函数式编程的方式使用核心的函数式编程操作，这将引起巨大的困惑。

你应该听过一句老话，用合适的工具做合适的事，对吗？锤子配钉子，螺丝刀配螺丝等等。这里有些细微的不同：采用**恰当的方式**使用合适的工具。
...
如果问题中的值时复合的，意味着它是由单个值组成，就像数组中的情况一样。例如，函子在每个单独的值上执行操作函数。函子实用函数创建的新值是所有单个操作函数执行的结果的组合。

这就是用`map(..)`来描述我们所看到东西的一种奇特方式。`map(..)`函数采用关联值（数组）和映射函数（操作函数），并为数组中的每一个独立元素执行映射函数。最后，它返回由所有新映射值组成的新数组。

另一个例子：字符串函子是一个字符串加上一个实用函数，这个实用函数在字符串的所有字符上执行某些函数操作，返回包含处理过的字符的字符串。参考如下非常造作的例子：
...
我们将这个筛选的过程称为“过滤”。将这次购物描述为从空篮子开始，然后只过滤（挑选，包含）出苹果和橙子，或者从所有的水果中过滤掉（跳过，不包括）香蕉。你认为哪种方式更自然？
...
我认为最通常的理解过滤（在编程之外）过滤是剔除掉不需要的成员。不幸的是，在程序中我们基本上将这个语意倒转为更像是过滤需要的成员。
...
注意，就像之前的`mapperFn(..)`， `predicateFn(..)`不仅仅传入了值，还传入了`idx` 和 `arr` 。如果有必要，采用 `unary(..)` 来限制它的形参。

正如`map(..)`，`filter(..)`也是JS数组内置支持的实用函数。
...
如果让你描述`[1,3,5]`这个结果，你是说“我将偶数过滤掉了”，还是说“我将奇数过滤了” ？我认为前者是更自然的描述。但后者的代码可读性更好。阅读代码几乎是逐字的，这样我们“过滤的每一个数字都是奇数”。

我个人觉得这语意混乱。 对于经验丰富的开发者来说，这里毫无疑问有大量的先例。但是对于一个新手来说，这个逻辑表达看上去不采用双重否定不好表达，换句话说，采用双重否定来表达。
...
**警告：** 在JavaScript中，如果在缩减操作的列表中没有一个值（在数组中，或没有指定`initialValue`），将会抛出异常。一个缩减操作的列表有可能为空的时候，需要小心采用不指定`initialValue`的方式。
...
在JavaScript中采用内建的`reduce(..)`方法表达列表的缩减操作：
...
就像`map(..)`和`filter(..)`， 缩减函数也传递不常用的`idx`和`arr`形参，以防缩减操作需要。我不会经常用到它们，但我觉得保留它们时明智的。

在第四章中，我们讨论了`compose(..)`实用函数，它被用于实现`reduce(..)`：
...
```
基于不同的组合，为了说明`reduce(..)`，可以认为缩减器将函数从做到右组合（就像`pipe(..)`做的事情）。在列表中这样使用：
...
```
不幸的是，`pipeReducer(..)`是非点自由的（见第三章中的“无点”），但我们不能仅仅以缩减器本身来传递`pipe(..)`，因为它是可变的；传递给`reduce(..)`额外的参数（`idx`和`arr`）会产生问题。
...
不像`map(..)`和`filter(..)`，对传入数组的次序没有要求。`reduce(..)`明确要采用从左到右的处理方式。如果你想从右到左缩减，JavaScript提供了`reduceRight(..)`函数，它和`reduce(..)`的行为出了次序不一样外，其他都相同。
...
**注意** 我们这个缩减器来欺骗，并允许采用`list.push(..)`去改变传入的列表所带来的副作用。一般来说，这并不是一个好主意，但我们清楚创建和传入`[]`列表，这样就不那么危险了。创建一个新的列表，并将val合并到这个列表的最后面。这样更有条理，并且性能开销较小。我们将在附录A中讨论这种欺骗。
...
**注意** 这里有更加不纯的缩减器欺骗。不采用`list.push(..)`，我们也可以采用`list.concat(..)`并返回合并后的新列表。我们将在附录A中继续介绍这个欺骗。
...
函数式编程的类库中，通常会定义一个`flatMap(..)`（通常命名为`chain(..)`）函数。这个函数将映射和之后的扁平化的操作组合起来。 For consistency and ease of composition (via currying), the `flatMap(..)` / `chain(..)` utility typically matches the `mapperFn, arr` parameter order that we saw earlier with the standalone `map(..)`, `filter(..)`, and `reduce(..)` utilities.
...
在Javascript中，对于函数式编程者来说，普遍感到失望的因素之一是统一策略来处理功能函数，但某些功能函数采用独立的函数提供。想想，我们在前面章节中推导的大量函数式编程式的功能函数，另一些功能函数时数组原型上的方法，就像这章中讨论的这些。

当你想合并多个操作的时候，这种痛苦会更明显：
...
两种方式的API实现了同样的功能。但它们的风格完全不同。很多函数式编程者更倾向采用后面的方式，但是前者在Javascript中毫无疑问的更常见。后者特别地让人不喜欢之处是后者采用嵌套调用。人们更偏爱链式调用，通常称为流利的API风格，被jQuery和一些工具采用。这种风格紧凑／简洁，并且从上往下声明，便于阅读。

这种用法影响可读性的原因是阅读顺序既不是从左到右（自上而下），也不是从右到左（自下而上），而是从里往外。

从右往左（自下而上）这两种风格自动组成规范的阅读顺序。因此为了探索这些风格隐藏的差异，我们特别地检查组合。看上去，它应当简洁。但这两种情况下都不方便。

### 链式组合方法

这些数组方法接收绝对的`this`参数，尽管在外在不能当一元来看，这使组合更加不便。为了应对这些，我需要一个`partial(..)`版本的`this`：
...
```
我们也需要一个版本的`compose(..)`，它调用链式上下文中每一个被部分应用的方法。它的输入值是前一部传入的（通过内在的this）：
...
```
但是，这种特别的独立风格遭受了来自自身的不便。层级数组上下文是第一个参数，而不是最后一个。因此我们需要采用从偏右应用来组合它们。
...
这就是为何函数式编程类库通常定义`filter(..)`, `map(..)`和`reduce(..)`交替采用最后一个参数接收数组，而不是第一个参数。它们通常必然招那些工具函数喜欢：
...
这种实现方式很整洁，这是函数式编程者为何喜欢独立的实用程序风格，而不是实例方法。但你的情况可能不同。

### 方法适配独立

在前面的`filter(..)` / `map(..)` / `reduce(..)`的定义中，你可能发现了这三个方法的共同点：它们都派发到相关的原生数组方法。 因此，我们能采用工具函数生成着三个独立适配函数吗？ 是的，让我们定义`unboundMethod(..)`来做这些：
...
**注意:** `unboundMethod(..)`在Ramda中称之为`invoker(..)`。
...
2. 定义一个以缩减函数方式适配工具函数，并且将其传递给`reduce(..)`实例方法。

**不要采用第一种方式** 扩展诸如`Array.prototype`的原生方法从来不是一个好主意，除非定义一个`Array`的子类。但是这超出了这里的讨论范围。未了阻止这种不好的方式，我们不会进一步去探讨这种方式。

让我们**关注第二种方式**。为了说明这点，我们将前面定义的递归实现的`flatten(..)`转换为独立工具函数：
...
var flatten =
	arr =>
		...
		, [] );
```
让我们将里面的`reducer(..)`函数抽取成独立的工具函数（并且调整它，让其独立于外部的`flatten(..)`运行）：
...
现在，我们可以将这个工具函数使用`reduce(..)`在数组方法中链式调用：
...
到此为止，大部分示例基于一列数字或者字符串，有点无聊，让我们讨论一些有亮点的列表操作：建模一个命令式的一系列语句声明。
...
首先，我们注意到定义了五个变量和一系列的`if`语句有效地守护着由`getCurrentSession()`, `getSessionId(..)`, `lookupUser(..)`, `getUserId(..)`, `lookupOrders(..)`, 和 `processOrders(..)`这六个函数调用组合。理想的，我们期望摆脱这些变量定义和命令式的条件。

不幸的是，在第4章中讨论的`compose(..)` / `pipe(..)`工具函数并没有提供给一个便捷的方式来表达在这个组合中的`!= null`条件。 让我们定义一个工具函数来解决这个问题：
...
这个映射的结果是组合的函数数组（的确，管道，在这个列出的顺序）。我们可以展开这个数组到`pipe(..)`，但由于我们已经做列表操作，让我们采用`reduce(..)`来处理。采用`getCurrentSession()`返回的会话值作为初始值：
...
接下来，我们观察到`getSessionId(..)`和`getUserId(..)`可以表达为映射子各自的值`"sessId"`和`"uId"`：

```js
[ "sessId", "uId" ].map( propName => partial( prop, propName ) )
```

但是为了使用这些，我们需要将另外三个函数（`lookupUser(..)`, `lookupOrders(..)`,和`processOrders(..)`）插入进来，用来获取上面讨论的那五个守护／组合的函数。

实现插入，我们可以将其当作列表合并。回顾本章前面介绍的`mergeReducer(..)`：
...
然后我们将`lookupOrders(..)`和`processOrders(..)`拼接到正在执行的函数数组中：
...
最后，将所有的合并到一起，将这些函数数组钉在早期的守护和组合上：
...
所有必要的变量声明和条件一去不返了，取而代之的是采用整洁和声明式的列表操作链接在一起。

如果你觉得现在的这个版本比之前要难，不要担心。 毫无疑问的，前面的命令式的形式，你可能更加熟悉。 你进化为函数式编程者的一步是开发一些具有函数式编程风格的代码，比如这些列表操作。随着时间推移，我们跳出这些代码，当你切换到声明式风格时更容易感受到代码的可读性。

在离开这个话题之前，让我们做一个真实的检查：这里的示例过于造作。不是所有的代码片段被简单的采用列表操作建模。务实的获取方式是本能的寻找这些机会，而不是过于追求代码的技巧；一些改进比没有强。经常退一步，并且问自己，是**提升了还是损害了**代码的可读性。

## 融合

当你将函数式列表操作更多的带入到代码的思考中，你可能会很快地开始看到链式组合行为，如：
...
想想在这个转换流程中的每一个值。在`words`列表中的第一个值，开始为`"Mr."`，变为`"Mr"`，然后为`"MR"`，然后通过`elide(..)`不变。另一个数据流为：`"responsible"` -> `"responsible"` -> `"RESPONSIBLE"` -> `"RESPONS..."`。
...
这是另一个`pipe(..)`能更便利的方式处理组合的场景，这样有序，可读：
...
如何融合两个以上的`filter(..)`谓词函数呢？ 通常视为一元函数，它们似乎适合组合。但是有个小问题，每一个函数返回了不同类型的值（`boolean`），这些返回值并不是下一个函数需要的输入参数。 融合相邻的`reduce(..)`调用也是可能的，但缩减器并不是一元的，这也会带来不小的挑战。我们需要更复杂的技巧来实现这些融合。我们将在附录A的“转换”中讨论这些高级方法。
...
在这个树形结构中，`banana`是跟节点，这棵树可能采用不同的方式创建节点，但其依旧可以采用二叉搜索树一样的方式访问。
...
**注意** 采用递归处理二叉树更自然。我们的`forEach(..)`工具采用递归调用自身来处理左右字节点。我们将在后续的章节章深入讨论递归。

回顾在本章开头描述的`forEach(..)`， 它存在有用的副作用，通常函数式编程期望有这个副作用。在这种情况下，我们仅仅在IO的副作用下使用`forEach(..)`，因此它是完美的理想的辅助函数。
...
`BANANA`和`banana`是一个不同的树（所有的节点都不同），就像在列表中执行`map(..)`返回一个新的数组。就像其他对象／数组的数组，如果`node.value`本身是某个对象／数组的引用，如果你像做深层次的转换，你需要在映射函数中手动的复制它。

如何处理`reduce(..)`？相同的基本处理过程：有序遍历树的节点的方式。一种可能的用法是`reduce(..)`我们的树得到它的值的数组。这对将来适配其他典型的列表操作很有帮助。或者，我们可以`reduce(..)`我们的树，得到一个它由所有的产品名合并的字符串。
...
最后，让我们考虑在树中用`filter(..)`。这个算法迄今为止最棘手，因为它有效（实际上没有）影响从树上删除节点，这需要处理几个问题。不要被这种实现吓到。如果你喜欢，现在跳过它，而关注我们如何使用它。
...
这段代码的大部分是专门处理当重复的树形结构中的节点被“删除”（过滤掉）的时候，移动节点的父／子引用。
...
你会在简单列表中使用本章大多数的列表操作。但现在你发现这个概念适用于你可能需要的任何数据结构和操作。函数式编程可以广泛应用在许多不同的场景，这是非常强大！
...
列表通常以数组展现，单它也可以作为任何数据结构表达／产生一个有序的值集合。因此，所有这些“列表操作”都是“数据结构操作”。
