...
关于 `const` 关键字和“常量”只关心赋值而不关心数据语义的特性是个又臭又长的故事。几乎所有语言的高级开发者都踩 `const` 地雷。事实上，Java 最终不赞成使用 `const` 并引入了一个全新的关键词 `final` 来区分“常量”这个语义。
...
其实只要查看一下在 `let x = 2`; 之后的几行代码，就可以判断出 x 这个变量是否被重新赋值过了。对我来说，“实际上不进行重新赋值”相对“使用容易迷惑人的 `const` 关键字告诉读者‘不要二次赋值’”**是一个更明确的信号**。
