...
我们不仅用调用栈代替了 `for` 循环，而且用 `return`s 的形式在回调栈中隐式的跟踪增量的求和（ `total` 的间歇状态），而非在每次迭代中重新分配 `total`。通常，FPer 倾向于尽可能地避免重新分配局部变量。

在我们总结的这种基本算法里，这些差异是微乎其微的。但是，随着算法复杂度的提升，你将更加能体会到递归带来的收益，而不是这些命令式状态跟踪。
