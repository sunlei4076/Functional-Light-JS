...
如果你写过一个库来创造一对函数的配对状态——两种中的一个在第一个片段调用 `studentRecord(..)` 另一个在第二个片段调用 `StudentRecord.bind(..)`——你可能更多的关心它们两的性能怎样。从代码角度，我们可以看到前一种每次创建一个新的函数表达式。第二种使用 `bind(..)`，没有明显的含义。
...
在库的使用案例之外，函数的配对状态通常在应用的关键路径发生的次数相对非常少。相比之下，典型的使用是函数加状态——在任意一个片段调用 `student()`——更加常见。
