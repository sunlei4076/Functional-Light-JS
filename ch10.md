...
到目前为止，我们所说的一切都是同步的，意味着我们调用函数【且传入参数】后马上就会得到返回值。大部分的情况下是没问题的，但这几乎满足不了现在的所有 JS 应用。为了能在现在的 JS 环境里使用上函数式编程，我们就需要去了解异步的函数式编程。
...
这可能不太明显，但是这个代码里还有其他内在的竞争状态，promise 的定义没有被体现出来。如果 `orders` 的查询在把 `onOrders(..)` 回调函数传给 `ordersPromise.then(..)` 调用前完成，那么就需要一些比较智能的*逻辑* 来保存 `orders` 直到 `onOrders(..)` 能被调用。 同理，`record` （或者说`customer`）对象是否能在 onCustomer(..) 执行时被接收到。

这里的 *某些东西* 同样是我们之前讨论过的时间复杂度逻辑类似。但我们不必去担心这些复杂性，无论是编码或者是读（更为重要）这些代码的时候，因为对我们来说，promise 所处理的就是时间上的问题。

promise 以时间无关的方式来作为一个单一的值。此外，获取 promise 的返回值是异步的，但却是通过同步的方法来赋值即时值。或者说， promise 给 `=` 操作符扩展随时间动态赋值的功能，通过可靠的（时间无关）方法。

接下来我们将探索如何以相同地方式，在时间上异步地拓展本书之前的同步的函数式编程操作。

## 积极的 vs 慵懒的

积极的 和 慵懒的 在计算机科学的领域并不是表扬或者批评的意思，而是描述一个操作是否立即执行或者是延时执行。

我们在本例子中看到的函数式编程操作可以被称为 积极的，因为它们在一个离散的即时值或值的列表/结构上同步（即时）操作。
...
为了理解如何在2个值之间创建和使用 慵懒的 映射，我们需要去抽象我们对列表（数组）的想法。
...
至此，这段代码的数组和普通的没有什么区别。唯一奇怪的是在我们执行 `map(..)` 来映射数组 `a` 生成数组 `b` 之后，定时器在 `a` 里面添加随机的值。

但是这个虚构的 `LazyArray` 有点不同，它假设了值可以随时的一个一个添加进去。就像随时可以 `push(..)` 你想要的值一样。可以说 `b` 就是一个懒映射 `a` 最终值的数组。

此外，当 `a` 或者 `b` 改变时，我们不需要确切的保存里面的值，这个特殊的数组将会保存它所需的值。所以这些数组不会随着时间而占用更多的内存，这是 懒数据结构和懒操作的重要特点。事实上，它看起来不像数组，更像是buffer（缓冲区）。

普通的数组是积极的，所以它会立马保存所有它的值。"懒数组" 的值则会延迟保存。
...
从 `b` （消费者）的角度来说，我们不用知道或者关注 `a` 里面的值在何时何地来的。事实上，所有的值都已经存在。我们只关注是否无论何时都能取到那些值。或者说，`map(..)` 的转换操作是一个无时态（lazy）的建模过程。
...
这本书详细的介绍了各种各样的函数式编程操作，例如：把单个值（或者说是一个立即列表的值）转换到另一个值里。

对于那些有时态的操作，所有基础的函数式编程原理都可以无时态的应用。就像 promise 创建了一个单一的未来值，我们可以创建一个 积极的 列表的值来代替像 慵懒的 observable（事件）流的值。

数组的 `map(..)` 方法运行是会映射一次当前数组里的所有的值，然后放到返回的数组里。observable 的数组则是映射一次所有的值，且监听到值什么时候进来，就会把它映射到返回的 observable 里。

或者说，如果数组对函数式编程操作是一个 积极的 数据结构，那么 observable 相当于持续 慵懒的。
