...
读到这里，你已经学习了所有我所说的轻量级函数式编程的基础概念，在本章节中，我们将把这些概念应有到不同的情景当中，但不会有新的知识点产生。
...
到目前为止，我们所说的一切都是同步的，意味着我们输入参数调用函数后立即得到返回值。大部分的情况下是没问题的，但几乎满足不了当前所有的 JS 应用。为了在真实的 JS 世界里真正准备好 FP ，我们需要去了解异步的 FP。

Our goal in this chapter is to expand our thinking about managing values with FP, to spread out such operations over time.

本章的目的是拓展我们对函数式编程的理解，以便之后我们在更多的业务上应用。
...
在你的所有应用里，最复杂的状态就是时间。我们可以很容易的管理那些状态改变时是立即和确定的。但是，如果状态随着时间和事件而隐晦的变化，管理这些状态的难度将会成几何级增长。
...
在例子中介绍的任何 FB 的部分都可以通过可靠性和可预见性，让代码变得更容易理解。但是当你添加异步到你的项目里，那将会是困难重重。
...
<del>但是让我们更明确一点：并不是说一些操作不能当作同步行为，触发异步行为很容易。这些操作的响应是协调的，每个操作都有可能改变应用程序的状态，这需要大量额外的工作。</del>
...
所以，作为作者的你最好付出一些努力，或者只是留给阅读你代码的人一个难题，去弄清楚如果A在B之前完成，项目中状态是什么，还有相反的情况呢？这是一个浮夸的问题，但以我的观点来看，这有一个确切的答案：有希望把复杂的代码变得更容易理解，作者就必须花费更多心思。
...
为了了解这一点，让我们先来看下有竞争状态的一种情况，且必须手动去管理。
...
回调函数 `onCustomer(..)` 和 `onOrders(..)`  之间是互为竞争关系。假设他们都在运行，2者都有可能先运行，那将无法预测到会发生什么。
...
如果我们可以把 `lookupOrders(..)` 写到 `onCustomer(..)` 里面，那我们就可以确认 `onOrders(..)` 会在 `onCustomer(..)` 之后运行，但我们不能这么做，因为我们需要让 2 个查询能被同时被调用
...
所以，为了让这个依赖时间的复杂状态变得正常，我们用相应的 `if`-声明来检测各自的回调函数里的外部作用域变量 `customer`。当各自的回调函数被执行，将会去检测 `customer` 的状态，从而确定各自的执行顺序；如果 `customer` 在回调函数里还没被定义，那他就是先运行的，其他情况下则是第二个运行的。
...
现在 `onOrders(..)` 回调函数存在 `onCustomer(..)` 里，所以他们各自的执行顺序是可以保证的。在指定的 `then(..)` 响应之前 `lookupCustomer(..)` 和 `lookupOrders(..)` 被分别的调用，从而保证了2个查询是并行的。
...
<del>这可能不太明显，但是这个片段里还有其他内在的竞争状态，promise 里的定义没有被体现出来。如果 `orders` 的查询在 `ordersPromise.then(..)` 的 `onOrders(..)` 调用前完成，那么需要*某些东西* 能足够聪明来保存 `orders` 直到 `onOrders(..)` 能被调用。事实上，同样的问题也出现在 `record` （或者说`customer`）是否有保存到被 `onCustomer(..)` 接受到</del>
...
这里的 *某些东西* 同样是我们之前讨论过的时间状态复杂的理由。但我们不必去担心这些复杂性，无论是编码的时候或者是读（更为重要）这些代码，因为对我们来说，promise 所处理的就是时间上的问题。
...
promise 以时间无关的方式表示为一个单一的值。此外，从 promise 返回的值是通过同步赋值的异步即时值。或者说， promise 给 `=` 操作符扩展随时间动态赋值的功能，通过可靠的（时间无关）的方法。
...
现在我们将探索如何在同一时间内异步的扩展各种同步的 FP 操作。
...
Eager 和 lazy 在计算机科学的领域并不是表扬或者批评的意思，而是描述一个操作是否立即完成或者随着时间的推移而进行着。
...
我们在本例子中看到的 FP 操作可以被称为 eager，因为他们的运算都是同步的即时的值或者是列表/结构（？）的值。
...
这里 `a` 到 `b` 的映射是 eager，因为它在某一刻映射了数组 `a` 里的所有的值，然后生成了一个新的数组 `b` 。即时之后你去修改 `a` ，比如说添加一个新的值到数组的最后一位，也不会影响到 `b` 的内容。这就是 eager FP。

But what would it look like to have a lazy FP operation? Consider something like this:

但是如果是一个 lazy FP 的操作呢？考虑下下面的情况：
...
我们可以想象下 `mapLazy(..)` 本质上“监听”了数组 `a` ，只要一个新的值添加到数组的后面（使用 `push(..)` ），它都会运行映射函数 `v => v * 2` 并把改变的值添加到数组 `b` 里。
...
**注意：** `mapLazy(..)` 的实现没有被写出来，因为它是虚构的，而不是一个真实。如果要实现 `a` 和 `b`之间的 lazy 操作，简单的数组需要变得更聪明。
...
考虑下把 `a` 和 `b` 联系到一起的好处，无论何时何地，你添加一个值进 `a` 里，它都将改变且映射到 `b` 里。<del>它比同为声明式 FP 的 `map(..)` 更强大</del>，但现在它可以随着时间而变化，你不必知道 `a` 所有的值来进行映射
...
为了理解如何创建和在2个值之间使用 lazy mapping，我们需要去抽象我们的列表（数组）。
...
让我们来想象一个聪明的数组，不只是简单的拥有变量，还是一个懒惰地接受和响应（也就是“反应”）变量的数组。考虑下：
...
至此，这段代码和普通的数组没有什么区别。唯一的不同点是我们习惯的用 `map(..)` 来映射数组 `a` 里当前的变量，eagerly 和立即产生数组 `b` ，但奇怪的是定时器在 `map(..)` 调用*之后*往 `a` 里面添加随机变量。
...
<del>但是这个虚构的 `LazyArray` 有点不同，它假设变量可以随着时间而出现一次。就像 `push(..)` 变量当你想要的时候。 `b` 是一个 lazy 映射到 `a` 最终值的数组。</del>
...
此外，当 `a` 或者 `b` 改变时，我们不需要确切的保存里面的变量，这个特殊的数组将会保存它所需的变量。所以这些数组不会随着时间而增长内存，这是 lazy 数据结构和操作的重要特殊。事实上，它看起来不像数组，更像是流。
...
<del>普通的数组是 eager 所以他会立马保存所有它的变量。一个 "lazy array" 则是可能随时改变它的变量。</del>
...
由于我们不一定知道 `a` 添加了新的变量，所以另一个关键就是我们需要有能力去监听 `b` 并在有新变量的时候通知它。我们可以想象下监听器是这样的：
...
<del>`b` 是反应性的，因为它被设置为当 `a` 有变量添加时进行*反应*。有一个 FP 的操作 `map(..)` 作用是把数据源 `a` 里面的所有变量转移到目标 `b`。每次单独的映射操作都是我们使用普通的同步 FP进行单值模型操作，但是在这里我们将随着时间来展开原始的变量</del>
...
**注意：** 最常用到这些概念的术语是 Functional Reactive Programming (FRP)。我故意避开这个术语是因为一个有关于 FP + Reactive 是否真的构成FRP的辩论。我们不会全面深入了解FRP的所有含义，所以我会继续称之为 reactive FP。或者，如果你不会感觉那么困惑，也可以称之为 evented-FP。
...
我们可以把 `a` 当作生成变量而 `b` 则是消费变量。所以为了可读性，我们得重新整理下这段代码，让问题归结于 *生产者* 和 *消费者*。
...
`a` 是一个行为本质上像数据流的生产者。我们可以把每个变量赋值给 `a` 当作一个*事件*。`map(..)` 操作会触发 `b` 上面的 `listen(..)` 事件来消费新的变量。
...
我们分离*生产者* 和 *消费者*的原因是因为我们的应用程序的不同部分都应该有各自职责。这样的代码组织可以很大程度上提高代码的可读性和维护性。
...
我们非常谨慎地在讨论中介绍时间状态。具体来说，正如 promise 把我们担心的时间状态从单个异步操作中抽离出来，reactive FP 则是从一系列的变量/操作中抽离（分割）了时间状态。
...
从 `b` （消费者）来说，我们不用知道或者关注 `a` 里面的变量在何时何地来的。事实上，所有的变量都已经存在。我们的关注点只在我们想要的那些变量，每当变量准备好。再者，这是 `map(..)` 转换操作生成的一个无时态（lazy）的模型。
...
以 operations-over-time 这种方式来组织变量可能不是很有效。让我们来对比下相同的功能如何表示出来：
...
在后面的命令式代码中，以消费者的角度来说它并不清楚 `v` 从哪里来。此外命令式强硬的把代码 `b.onValue(..)` 夹杂在生产者 `a` 的逻辑里，这有点违反了 separation-of-concerns（职责分离？）。这将会让分离生产者和消费者变得困难。
...
相比之下，在之前的代码中，`b = a.map(..)` 表示了 `b` 的变量是来源于 `a` 的，对于如同抽象事件流的数据源 `a`，我们不需要关心了。我们可以*断言*任何来自于 `a` 到 `b` 里的变量都会通过 `map(..)`操作。
...
为了方便，我们已经举例了这个概念：随着时间推移，通过不断的 `map(..)` 来绑定 `a` 和 `b`。但是我们有许多其他的 FP 操作可以运行的很好。
...
这里可以看到 `a` 的变量肯定会通过 `isOdd(..)` 到 `b` 里面。
...
因为我们调用 `reduce(..)` 是没有给具体的 `initialValue` 的值，无论是 `sum(..)` 或者 `runningTotal(..)` 都会等到有 2 个来自 `a` 的参数时才会被调用。
...
这段代码暗示了在 reduction 里面有一个 *内存空间*， 每当有新的变量进来的时候，`sum(..)` 都会被调用且带上第一个参数 `total` 和第二个参数 `v`。
...
其他的 FP 操作如同内在的缓存，比如说 `unique(..)` 可以追踪每一个它访问过的值。
...
**注意：** 只是做些假设（希望）：接下来的讨论只是简要的介绍 observables。这是一个需要我们花充足的时间去探究的更深层次的话题。但是如果你理解本文中的轻量级函数式编程，并且知道如何通过 FP 原理来构建异步的话，继续学习 observables 将会变得顺心得手。
...
在 RxJS 中，一个 Observer 订阅一个 Observable。如果你把 Observer 和 Observable 的功能结合到一起，那就会得到一个 Subject。因此，为了保持代码的简洁，我们把 `a` 构建成一个 Subject，所以我们可以调用它的 `next(..)` 来添加变量（事件）到他的数据流里。
...
除了 `map(..)`，RxJS 还定义了超过 100 个可以在有新的变量添加时才触发的方法，就像数组一样。每个 Observable 的方法都会返回一个新的 Observable，意味着他们是链式的。如果一个方法被调用，则它的返回值应该由输入的 Observable 去返回，然后触发到输出的 Observable里，否则抛弃。
...
这本书详细的介绍了各种各样的 FP 操作，例如：获取单个变量然后把他转换到另一个变量里。
...
对于那些有时态的操作，所有的 FP 基础原则都可以应用到与时间无关的方面。就像单一的未知变量 promise 模型，我们可以创建一个 eager 列表的变量来代替像 lazy  observable（事件）流的值。
...
数组的 `map(..)` 方法运行是会映射一次当前数组里的所有的变量，然后放到返回的数组里。observable 的数组则是映射一次所有的变量，无论变量什么时候进来，然后映射变量到返回的 observable 里。

In other words, if an array is an eager data structure for FP operations, an observable is its lazy over-time counterpart.

或者说，如果数组对 FP 操作是一个 eager 的数据结构，那么 observable 相当于 时态的 lazy。