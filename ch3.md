...
在第二章的 “函数输入” 小节中，我们聊到了函数形参（parameters）和实参（arguments）的基本知识，甚至还了解到一些能简化其使用方式的语法技巧，比如 `...` 操作符和解构（destructuring）。
...
提一句，对于这类代码，FPer 往往喜欢使用更简短的 `=>` 箭头函数语法（请看第二章的 “语法” 小节），像这样：
...
因为 `add(..)` 函数不是 `map(..)` 函数所预期的遍历函数，所以我们不直接把它传入 `map(..)` 函数里。这样一来，偏应用就有了用武之地：我们可以调整 `add(..)` 函数签名，以符合 `map(..)` 函数的预期。
